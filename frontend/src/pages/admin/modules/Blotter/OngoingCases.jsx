...
const StatusBadge = ({ status, type = 'default' }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium shadow-sm border ${config.color} ${
...
const NoShowBadge = ({ type, count = 0 }) => {
  const config = {
    complainant: {
      color: 'bg-red-100 text-red-800 border-red-200',
      icon: <UserMinusIcon className="w-3 h-3" />,
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium shadow-sm border ${color} animate-pulse`}>
      {icon}
    {text}
        ...
        <div className="w-full max-w-[95%] xl:max-w-[1800px] mx-auto space-y-8">
          ...
          <div className="text-center space-y-4">
            <div className="flex items-center justify-center gap-4 mb-4">
              <button
                onClick={() => handleNavigation('/admin/blotter-records')}
                    ...
                    {getPaginatedRecords(getSortedRecords(getFilteredRecords())).length === 0 ? (
                      ...
                      getPaginatedRecords(getSortedRecords(getFilteredRecords())).map((record, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-4">
                            <div className="transform group-hover:scale-105 transition-transform duration-300 space-y-1">
                              ...
                              {record.complainant_appeal_submitted && (
                                <div className="flex items-center gap-1 text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded-full">
                                  ...
                                  <span>Complainant Appeal: {record.complainant_appeal_status || 'Pending'}</span>
                                </div>
                              )}
                              {record.respondent_appeal_submitted && (
                                <div className="flex items-center gap-1 text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full">
                                  <DocumentTextIcon className="w-3 h-3" />
                                  <span>Respondent Appeal: {record.respondent_appeal_status || 'Pending'}</span>
                                </div>
                              )}
                            </div>
                          </td>
